...
int hpmdownload(unsigned char *byte, unsigned int filesize, unsigned char *ip, unsigned char *username, unsigned char *password, unsigned char slot){
	unsigned char i;
	...
	for(i=0; i < img_info.nb_actions; i++){
		if(img_info.actions[i].action == 0x02){
			switch(hpm_upgrade(ip, username, password, slot, &img_info.actions[i], byte)){
...
unsigned char check_hpm_info(unsigned char *ip, unsigned char *username, unsigned char *password, unsigned char amc_slot_number){
    ...
    intf->close(intf);
      ...
      intf->close(intf);
		...
		if(rsp->data_len != 11){intf->close(intf);	return 0xFD; }
		
		if(rsp->data[9] != img_info.product_id[0] || rsp->data[10] != img_info.product_id[1]){intf->close(intf);	return 0xFC; }	//Check product ID
		if(rsp->data[6] != img_info.manufacturer_id[0] || rsp->data[7] != img_info.manufacturer_id[1] || rsp->data[8] != img_info.manufacturer_id[2]){intf->close(intf);	return 0xFB;	}//Check Manufacturer ID
		if(rsp->data[2] < img_info.earliest_compatibility_vers[0] || (rsp->data[2] == img_info.earliest_compatibility_vers[0] && rsp->data[3] < img_info.earliest_compatibility_vers[1])){intf->close(intf); return 0xFA; }//Check vers.		
	}
    ...
    intf->close(intf);
      ...
      intf->close(intf);
		...
		if(rsp->data_len != 8){ intf->close(intf);	return 0xF8; }
		
		if(rsp->data[1] != 0x00){ intf->close(intf);	return 0xF7;	}//HPM.1 not supported
		if(rsp->data[2] & 0x01){ intf->close(intf);		return 0xF6;	}//Firmware upgrade is not desirable at this time
		...
		if((rsp->data[2] & 0x07) != (img_info.image_capabilities & 0x07)){ intf->close(intf);	return 0xF5;	}//Capabilities are different between HPM image and MMC's information
		if((rsp->data[7] & img_info.components) !=  img_info.components){intf->close(intf); 	return 0xF4;	}//Component(s) not present
...
unsigned char hpm_upgrade(unsigned char *ip, unsigned char *username, unsigned char *password, unsigned char amc_slot_number, action_t *action, unsigned char *byte){
	unsigned char len, i;
	...
	data[1] = 0x00; //action-> components;		//Component (only one for upgrade action)
	...
	do{
      ...
      intf->close(intf);
			...
			if(rsp->ccode == 0x80 || rsp->ccode == 0xc3){ usleep(10000); }	
			else if(rsp->ccode == 0x00){	ccode = rsp->data[2]; usleep(10000);	}
			else{	usleep(10000);	}
		}
		timeout++;
		
	}while(rsp != NULL && timeout < (5 * img_info.inaccessibility_timeout * 100) && ccode != 0x00);
	...
	if(rsp == NULL){
		...
		printf("PICMG id: 0x%02x \n", rsp->data[0]);
		printf("cmd in progress: 0x%02x \n", rsp->data[1]);
		printf("last completion code: 0x%02x \n", rsp->data[2]);
		printf("comp estimate: 0x%02x \n", rsp->data[3]);
	}
	*/
	
	//printf("timeout : %d / %d | ccode = 0x%x \n", timeout, (5 * img_info.inaccessibility_timeout * 100), ccode);
	if(timeout >= (5 * img_info.inaccessibility_timeout * 100)){intf->close(intf);	return 0xFC; }
	...
	for(offset=0, block_nb=0; offset < action->firmware_length; block_nb++){
		...
		if(rsp == NULL){
			...
			while(rsp != NULL && rsp->ccode == 0xc3 && retry < 10){
				block_nb++;
				if(block_nb == 0xFF) block_nb = 0x00;				
				data[1] = block_nb;
			...
			if(rsp->ccode != 0x82 && rsp->ccode){	//Error (0x82 means that packet has already been received)
				
				printf("\n[INFO] \t {UPLOAD_FIRMWARE_BLOCK} \t Completion Code : 0x%02x \n", rsp->ccode);
				intf->close(intf);
				return 0xFA;
			}	
		}
