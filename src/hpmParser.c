...
unsigned char *hpm_parse(unsigned char *binary, int binsize, unsigned int *hpmsize, unsigned char *iana, unsigned char *prodid, unsigned char earliest_major, unsigned char earliest_min, unsigned char new_maj, unsigned char new_min){
	...
	if(img == NULL){
  	...
  	offset += header(img, 0, iana, prodid, earliest_major, earliest_min, new_maj, new_min);
  
	offset += upgrade_action(img, offset, binary, binsize);
...
int header(unsigned char val[], unsigned offset, unsigned char *iana, unsigned char *prodid, unsigned char earliest_major, unsigned char earliest_min, unsigned char new_maj, unsigned char new_min){
	unsigned timestamp, i;
	char crc = 0;
...
int prepare_action(unsigned char val[], int offset){
...
int upgrade_action(unsigned char val[], int offset, unsigned char *binary, int binsize){
	int i;
